...
class App:
    ...
    def reset_game(self): #機械の定義　機械は関数 ゲームの状態を初期化するための関数です。ゲームをリセットし、新しいゲームを開始できるように、すべてのグローバル変数を初期状態に戻します
    # globalはグローバル変数を使う宣言
    ...
    def update_power(self): #ゲーム内のパワーゲージを更新するための関数。ゲージの増減、毛抜の増減を遅くする
